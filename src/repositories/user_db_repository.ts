...
export class UserRepository {
    ...
    static async getUserById(id:string): Promise<UserModel | null>{
        const user = await UserModelMongoose.findOne({_id: new ObjectId(id)})
        if(!user){
            return null
        }
    ...
    static async getConfirmationInfo(email: string):Promise<{confirmationCode: string, expirationDate: string, isConfirmed: boolean}|null>{
        const info = await UserModelMongoose.findOne({email: email})
        if(!info){
            return null
        }
    ...
    static async getUserByConfirmationCode(code:string):Promise<WithId<UserDbType>|null>{
        const user = await UserModelMongoose.findOne({"emailConfirmation.confirmationCode": code})//{emailConfirmation: {confirmationCode: code}}
        if(!user) return null
        return user
    }

    static async getUserWithPassword(loginOrEmail:string): Promise<WithId<UserDbType>| null>{
        const user  = await UserModelMongoose.findOne({$or: [{login: loginOrEmail}, {email: loginOrEmail}]})
        if(!user){
            return null
        }
        return user
    ...
    static async getUserByLogin(login: string): Promise<UserModel| null>{
        const user  = await UserModelMongoose.findOne({login: login})
        if(!user){
            return null
        }
        return userMapper(user)
    ...
    static async getUserByEmail(email: string): Promise<UserModel| null>{
        const user  = await UserModelMongoose.findOne({email: email})
        if(!user){
            return null
        }
        return userMapper(user)
    ...
    static async addUser(newUser: UserDbType): Promise<UserModel|undefined>{
        const user = await UserModelMongoose.insertMany([newUser])
    ...
    static async deleteUserById(id:string): Promise<boolean> {
        const user = await UserModelMongoose.deleteOne({_id: new ObjectId(id)})
        return !!user.deletedCount;
    }

    static async deleteUserByEmail(email:string): Promise<boolean> {
        const user = await UserModelMongoose.deleteOne({email: email})
        return !!user.deletedCount;
    }

    static async updateUserConfirmation(id: string): Promise<boolean>{
        const user = await UserModelMongoose.updateOne({_id: new ObjectId(id)},
